...
'1day';;
;'6day';;
runDate='20130316' ;20090115';
sw_output2file=0 ;1:PNG' ;0:NONE';
TEST = $
;'4to8';
;'swNeutral1O'
'swNeutral1'
...
;'/scratch3/NCEPDEV/swpc/save/Tzu-Wei.Fang/IPE/trunk_electrodynamics_44514_6/run/ipe_tgcm_4to8UT/'
;'/scratch3/NCEPDEV/swpc/save/Tzu-Wei.Fang/IPE/trunk_electrodynamics_44514_6/run/ipe_tgcm_0to4UT/'
;'/scratch3/NCEPDEV/swpc/scrub/Gerhard.Theurich/rt_42695/swpc%20090115_1day_spacewx_gsm%wam%T62_ipe%80x170/' ;sw_neutral=3
;'/scratch3/NCEPDEV/swpc/scrub/Gerhard.Theurich/rt_122363/swpc%20090115_1day_spacewx_gsm%wam%T62_ipe%80x170/' ;sw_neutral=1
;'/scratch3/NCEPDEV/swpc/scrub/Gerhard.Theurich/rt_137188/swpc%20090115_1hr_spacewx_gsm%wam%T62_ipe%80x170/' ;sw_neutral=1, only T on
;'/scratch3/NCEPDEV/swpc/noscrub/Naomi.Maruyama/ipe/reu/tmp20130703reu/trunk/run/ipe_640_18702/';old reu
;'/scratch3/NCEPDEV/swpc/scrub/Gerhard.Theurich/rt_122363/swpc%20090115_1day_spacewx_gsm%wam%T62_ipe%80x170/'
...
if ( sw_read_wind eq 1 ) then begin
   ...
   print,dirtmp7
;
   flnmtmp7=dirtmp7+'ut_out4wind' ;wam-ipe
   ;flnmtmp3=dirtmp7+'wind_out'    ;wam-ipe
   flnmtmp8=dirtmp7+'tn_out'      ;tn
   flnmtmp9=dirtmp7+'on_out'      ;on
...
plot_DIR= $
;fig_DIR
;fig_DIR+'rt_'+rtNumber+'/'
fig_DIR+'fig/wam_ipe/'
;fig_DIR+'fig/'+TEST+'/'+rtNumber+'/'
;fig_DIR+'fig/'+rundir+'/'
;if ( sw_dif eq 1 ) then $
...
;dbg20141208
TE_TI_k_save  =fltarr(3,NPTS2D,NMP)
...
;d print,'call read_wind: JMIN_IN',JMIN_IN
  if ( sw_read_wind eq 1 ) then $
     read_wind, UT_hr $
                ,Vn_ms1,tn_k,on_m3,n2n_m3,o2n_m3 $       ;input to IPE
...
;dbg20170503
flnmtmp100='NaNOnm3.dat'
openw,luntmp100,flnmtmp100,/GET_LUN
print,'!dbg20170503! opening file',flnmtmp100
for mp=0,nmp-1 do begin
for lp=0,nlp-1 do begin

ipts=jmin_in[lp]-1
printf,luntmp100, n_read,mp,lp,ipts,(ipts-JMIN_IN[lp]),xionn_m3[4,ipts,mp],z_km[ipts],on_m3[ipts:ipts+4,mp],glon_deg[ipts,mp],glat_deg[ipts,mp],tn_k[ipts,mp],n2n_m3[ipts,mp],o2n_m3[ipts,mp],FORMAT='("NH",i2,i3,i4,i7,i6,e12.4,f6.0,5e12.4,2f7.0,f6.0,2e12.4)'

ipts=jmax_is[lp]-1
printf,luntmp100, n_read,mp,lp,ipts,(ipts-JMIN_IN[lp]),xionn_m3[4,ipts,mp],z_km[ipts],on_m3[ipts-4:ipts,mp],glon_deg[ipts,mp],glat_deg[ipts,mp],tn_k[ipts,mp],n2n_m3[ipts,mp],o2n_m3[ipts,mp],FORMAT='("SH",i2,i3,i4,i7,i6,e12.4,f6.0,5e12.4,2f7.1,f6.0,2e12.4)'
...
;       plot_z[n_read,k,0,ipts] = (-1.)*Vn_ms1[jth,ipts,mp] 
       plot_z[n_read,2,0,ipts] = (-1.)*Vn_ms1[jth,ipts,mp] ;north-->southward ;20170222
endif ;( sw_read_wind eq 1 ) then begin
...
;dbg20160421
if  (sw_hourly_plot eq 1) AND ( (n_read MOD 30) ne 0 ) then continue
...
if ( sw_quickplot eq 0 ) then $
   ...
   ,VarType_max $
   ,VarType_step $
   ;, input_DIR0,TEST,  rtNumber, UserName $
else if ( sw_quickplot eq 1 ) then begin ;$
  rundir = 'ipe_'+UserName+'_'+rtNumber
  print,'rundir=',rundir
     ctr_lat_ht_quick    $ 
        , JMIN_IN,JMAX_IS,Z_km,mlat_deg  $ 
        ...
        ,VarType_step $
        ,n_read_max $
        , input_DIR0,TEST,  rtNumber, UserName, glon_deg2D, rundir $
,n_plt_min, on_m3
endif
...
ALOG10 ( XIONN_m3[2-1,in:is,MP_plot] * 1.0E-6 ) ;[h+]
     plot_x[i_window,2,0:FLDIM_plot[i_file]-1,i_file] =$
ALOG10 ( XIONN_m3[3-1,in:is,MP_plot] * 1.0E-6 ) ;[he+];v0
;ALOG10 ( ( XIONN_m3[4,in:is,MP_plot]  ) * 1.0E-6 ) ;[No+] ;v2
;ALOG10 ( ( XIONN_m3[5,in:is,MP_plot]  ) * 1.0E-6 ) ;[No+] ;v3
...
;for kk=in,in+40 do  print,kk, z_km[kk],XIONN_m3[3-1,kk,MP_plot]
     ...
     plot_x[i_window,3,0:FLDIM_plot[i_file]-1,i_file] =$
;ALOG10 ( TOTAL( XIONN_m3[*,in:is,MP_plot],1) * 1.0E-6 ) ;[Ne] ;v0
ALOG10 ( ( XIONN_m3[3,in:is,MP_plot] + XIONN_m3[4,in:is,MP_plot] + XIONN_m3[5,in:is,MP_plot] +XIONN_m3[6,in:is,MP_plot] +XIONN_m3[7,in:is,MP_plot] ) * 1.0E-6 ) ;[MIN+] ;v1
;ALOG10 ( ( XIONN_m3[3,in:is,MP_plot]  ) * 1.0E-6 ) ;[N+] ;v2
;ALOG10 ( ( XIONN_m3[6,in:is,MP_plot]  ) * 1.0E-6 ) ;[N+] ;v3
     i_window=1L
     plot_x[i_window,0,0:FLDIM_plot[i_file]-1,i_file] =   TE_TI_k[1-1,in:is,MP_plot] ;STORM Ti
     plot_x[i_window,1,0:FLDIM_plot[i_file]-1,i_file] = $
0.;  TE_TI_k[1-1,in:is,MP_plot] ;QUIET Ti ;v0
;TE_TI_k_save[1-1,in:is,MP_plot] ;v4 old Ti
;( TE_TI_k[3-1,in:is,MP_plot] - TE_TI_k_save[3-1,in:is,MP_plot] )*10. ;/ TE_TI_k_save[3-1,in:is,MP_plot] * 1.E6  ;v4 te % change
     plot_x[i_window,2,0:FLDIM_plot[i_file]-1,i_file] =   TE_TI_k[3-1,in:is,MP_plot] ;STORM Te

     plot_x[i_window,3,0:FLDIM_plot[i_file]-1,i_file] = $
0.;  TE_TI_k[3-1,in:is,MP_plot] ;QUIET Te ;v0
;TE_TI_k_save[3-1,in:is,MP_plot] ;v4
  endif 
...
;dbg20141208
;for kk=0,FLDIM_plot[i_file]-1  DO   print,'check diff Te',kk,  plot_x[1,3,kk,i_file]
...
;dbg20141208
print, 'saving old Te and Ti'
te_ti_k_save = te_ti_k


;nm20170208 plot plasmasphere for akebono comparison
