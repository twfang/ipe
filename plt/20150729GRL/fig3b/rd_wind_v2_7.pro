;20150205 v7: plot 14--17UT
;purpose: UT variation of wind at 302km at mp=80
;20150721v0: original to reproduce the fig3b in paper v0
;20150923v2: 5 lines per figure
;20160115
pro rd_wind_v2_7
...
;  y_max=3.0E+12*fac ;f107=120
  ...
  flnm_png=rundir+'noon_nmf2BW.'+version+'.png'
 ...
 flnm_png=rundir+'windBW.'+version+'.png'
endif
...
if ( sw_dot eq 1 ) then begin
   ...
   ;if (ut_hr-ut0) eq 2. then  line_style=5  else $
   if (ut_hr-ut0) eq 13. then  line_style=0  else $ ;nm20141008 new MIN
   if (ut_hr-ut0) eq 14. then  line_style=1  else $
   if (ut_hr-ut0) eq 15. then line_style=4  else $ 
   if (ut_hr-ut0) eq 16. then line_style=5  else $
   if (ut_hr-ut0) eq 17. then line_style=2  else $
;   if (ut_hr-ut0) eq 24. then line_style=0  else $
   begin
      print,'i=', i, line_style, line_thick 
      CONTINUE                  ;go to next i
   endelse
endif ;( sw_dot eq 1 ) then begin
...
n_ldct=39;33
loadct, n_ldct
; colory = col_min + (ut_hr-ut0)*(col_max-col_min)/24.;FIX(imax)
 colory = col_min + (ut_hr-13)*(col_max-col_min)/5.;FIX(imax)

print,'ut=', ut_hr, colory

;d if ( (ut_hr-ut0) eq 23. ) then print, y_plot[0:jmax]
...
if ( sw_dot eq 0 ) then tmp_string='line.'+version+'.png' else $
if ( sw_dot eq 1 ) then tmp_string='dot.'+version+'.png'
 flnm_png=rundir+'wind'+STRTRIM( string(n_ldct, FORMAT='(i2)'),1 )+tmp_string
print, flnm_png
output_png, flnm_png

print, 'rd_wind_v2_7 finished successfully'
end
