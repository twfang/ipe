...
sw_debug=0L
plot_DIR='../figures/discon/1dnewflipgrid/fort'+STRTRIM( string(sw_fort, FORMAT='(i3)'), 1)+'/'
n_file=4L
...
;FLDIMphil=401
;opening files
   open_fort168, n_file,LUN,sw_debug,title_hemi,lp_title,sw_fort
...
SUMION0 = dblarr(3,FLDIM)  ;SUMION(1,7,J)+SUMION(2,4,J)+SUMION(2,5,J)
 xionn0  = dblarr(4,FLDIM)
 eqn2d0 = dblarr(FLDIM)
nplsrd0 = dblarr(FLDIM)
...
;print, 'i', i,FLDIM
if ( i eq 0 ) then sw_version = 3  else $
...
;else  GOTO, JUMP1
     ...
     if ( (UT_hr[0]-UT_hr0_save) MOD freq_plot_hr LT 0.0001 ) then begin
...
;title_hemi='NH' ;requirement for j0/j1
       profile_ht_3d,plot_x,plot_y, title_hemi,mlat_title,ut_hr,lt_hr,plot_DIR,FLDIM_plot,mp_plot,sw_debug,sw_fort
