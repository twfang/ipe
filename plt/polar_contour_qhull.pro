;nm050107: QHULL is introduced, it works!!!
;nm012706:  copied from /usr/local/rsi/idl_6.2/lib/
...
PRO Polar_Contour_qhull, z, theta, r, SHOW_TRIANGULATION=show, DITHER=dither, $
         IRREGULAR=irregular, $
         _Extra = e $
,sw_dbg
;
...
;012706:
if ( sw_dbg eq 1 ) then  print, 'call triangulate1, 6.2'
...
if ( sw_dbg eq 1 ) then begin
   print, 'call triangulate2, 6.2'
   print, 'check SIZE X', size(x)
   print, 'check SIZE Y', size(y)
   print, 'check MAX',MAX(abs(x)), MAX(abs(y)), MAX([abs(x),abs(y)])
endif
TOLERANCE_value=0.0D0
TOLERANCE_value=(max(abs(x)) > max(abs(y)))*1.0E-12  
                ;error!!! MAX([abs(x),abs(y)])*1.0e-6
if ( sw_dbg eq 1 ) then  print, 'check SIZE: TOLERANCE',size(TOLERANCE_value)


;050107:
QHULL, x, y, tr, SPHERE = s
if ( sw_dbg eq 1 ) then  HELP, s, tr 
;050107:     triangulate, x, y, tr, TOLERANCE=TOLERANCE_value
;    triangulate, x, y, tr, TOLERANCE=0              ;032306: original
