...
!---------------------------
      ...
      INTEGER(KIND=int_prec) :: i,mp,lp, midpoint,ipts
...
!dbg20110923
IF( sw_debug )  print *,'sub-neut: mp=',mp,lp,IN,IS,npts
...
!dbg20110131: the midpoint values become NaN otherwise because of inappropriate D1/3 values...
               IF ( lp>=1 .AND. lp<=6 .AND. i==midpoint )   Un_ms1(:,i,mp) = Un_ms1(:,i-1,mp) 
...
print *,i,ipts,alt_km(ipts),Vn_ms1(1,ipts),Un_ms1(3,i,mp),glongi,glati,(decmag*57.296),BCOMPU
endif
...
!dbg20110923      print "(' un_meast_ms1      = ',2F10.4)",un_ms1(1,IN,mp), un_ms1(1,IS,mp)
!dbg20110923      print "(' un_down/eq_ms1   = ',2F10.4)",un_ms1(2,IN,mp), un_ms1(2,IS,mp)
...
! explicitly disaccociate the pointer
          NULLIFY (IN,IS)
