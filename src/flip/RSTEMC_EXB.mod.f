...
!dbg20120330
  ...
  !dbg20120330:
      DOUBLE PRECISION DIV_VEL(3)   !..terms for Div perp ExB(DIV_VEM)
      DOUBLE PRECISION DRFT_TRM     !..Integrated Div perp EXB term
...
!dbg20120330: div Vem term is re-included from FLIP
        !.. ExB drift term from Bailey's eqns. 16,19 in PSS,1983,p389
        DIV_VEL(I) = BOLTZ*DIV_VEM(K) *NE * TI(2,K)/BM(K)
      ENDDO
      ...
      !..20120330: drft_trm added, are the values after integration
...
!dbg20120330: div vem term is re-included from FLIP
      if ( sw_divv==0 ) then
         ...
         CALL TERLIN(DIV_VEL(1),DIV_VEL(2),DIV_VEL(3),DS_LO,DS_UP
     >  ,DRFT_TRM)
      end if
...
!dbg20120330: div Vem term is re-included from FLIP
        !.. ExB drift term from Bailey's eqns. 16,19 in PSS,1983,p389
        DIV_VEL(I) = BOLTZ*DIV_VEM(K) *NE * TI(3,K)/BM(K)
      ENDDO
