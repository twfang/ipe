...
!dbg20120330
      DOUBLE PRECISION DIVEM(2)        !.. div Vem
...
!dbgb20120330C----- Modified in August 1995 to include ExB drift term. Bailey et al.
!C----- PSS 1983, page 389 (DIVEM)
...
!dbg20120330: where can i get NSAVE???
C**sil ExB drift term is added.  dv(1,k) is for oxygen and dv(2,k) for
C     hydrogen
        if ( sw_divv==1 ) then
           DV(1,I)=div_vem(K)*(0.5*(N(1,K)+NSAVE(1,K)))/BM(K)
           DV(2,I)=div_vem(K)*(0.5*(N(2,K)+NSAVE(2,K)))/BM(K)
        end if
      ENDDO
...
!dbg20120330
        if ( sw_divv==0 ) then
          ...
          CALL TEREXP(DV(I,1),DV(I,2),DV(I,3),DS(J-1),DS(J),DM)
          DIVEM(I)=DM*eq_vperp
        end if
      ENDDO
