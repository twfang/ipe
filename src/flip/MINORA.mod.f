...
C.... Cleaned up and commented by P. Richards in April 200
!dbg20120330:C-- Modified in August 1995 to include ExB drift term. Bailey et al.
C-- PSS 1983, page 389 (DIVEM)
     ...
     >             EFLAG)    !.. OUTPUT: Error flag array
      ...
      USE module_input_parameters,ONLY:zlbnp_inp 
      IMPLICIT NONE
      ...
      ZLBHE=200.00         !.. set He+ lower boundary
!dbg20120304      ZLBNP=115         !.. set N+ lower boundary
      ZLBNP=zlbnp_inp         !.. set N+ lower boundary !dbg20120304:
...
!dbg20120330
      ...
      IF(IABS(IHEPNP).EQ.9) 
     >   CALL CHEMO9(NION,J,Q,N,NMSAVE,TI,L,DIVEM)
      IF(IABS(IHEPNP).EQ.11) CALL CHEM11(NION,J,Q,N,NMSAVE,TI,1,L,DIVEM)
...
!dbg20120330 Updated July 1998 to include ExB drift
...
!dbg20120330
        ...
        if ( sw_divv==1 ) then
           DV(1,K)=div_vem(J)*(0.5*(N(1,J)+NMSAVE(1,J)))/BM(J)
           DV(2,K)=div_vem(J)*(0.5*(N(2,J)+NMSAVE(2,J)))/BM(J)
        end if
 300  CONTINUE
...
!dbg20120330
        if ( sw_divv==0 ) then
           DIVEM(I)=0.0
        else if ( sw_divv==1 ) then
           CALL TERLIN(DV(I,1),DV(I,2),DV(I,3),DS(JI-1),DS(JI),DM)
           DIVEM(I)=DM*eq_vperp
        end if
 400  CONTINUE
      RETURN
...
C.... to do the interpolation.
!dbg20120330Updated July 1998 to include ExB drift
C.... See RSDENA_ExB.F for details. Bailey et al. PSS 1983,page 389 (DIVEM)
...
!dbg20120330
      ...
      USE module_WHICH???,ONLY:EQ_VPERP,DIV_VEM
...
!dbg20120330
        if ( sw_divv==1 ) then
        !.. Silvy ExB drift term is added. dv(1,k) is for He+/N+ and dv(2,k) for H+
           DV(1,K)=div_vem(J)*N(1,J)/BM(J)
           DV(2,K)=div_vem(J)*N(2,J)/BM(J)
        end if
