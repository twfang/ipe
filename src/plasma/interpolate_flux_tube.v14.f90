...
!NOTE:20110926:interpolation is done only for plasma density at this moment ...for debug purpose
...
mp_t0_loop: DO imp=1,imp_max
  mp0 = mp_t0(ihem,imp)
if(sw_debug) print *,imp,'mp_t0',mp_t0(ihem,imp),' mp0',mp0
...
!t  ELSE IF(jth==ISPEC+1) THEN
!t    Qint(jth, ip1d,imp,ilp) = (factor2*(plasma_3d(mp0,lp0)%Te_k(i1d-1) - plasma_3d(mp0,lp0)%Te_k(i1d))) + plasma_3d(mp0,lp0)%Te_k(i1d)
!t
!t  ELSE !Ti
!t
!t print *,'check jth for Ti_k:',jth,(jth-ISPEC-1)
!t IF ( (jth-ISPEC-1)>2 ) THEN 
!t print *,'!STOP! INVALID jth',ip1d,mp,lp,i,i1d,mp0,lp0,jth,(jth-ISPEC-1)
!t STOP
!t END IF
!t    Qint(jth, ip1d,imp,ilp) = (factor2*(plasma_3d(mp0,lp0)%Ti_k( (jth-ISPEC-1),i1d-1) - plasma_3d(mp0,lp0)%Ti_k( (jth-ISPEC-1),i1d))) + plasma_3d(mp0,lp0)%Ti_k( (jth-ISPEC-1),i1d)
!t   END IF
...
!ispecial=1: interpolation at/below IS_t0
      ELSE if(ispecial == 1) then
        ...
        !Te:
!t        Qint(ISPEC+1   ,ip1d,imp,ilp) = plasma_3d(mp0,lp0)%Te_k(         i1d)
        !Ti:
!t        Qint(ISPEC+2:iT,ip1d,imp,ilp) = plasma_3d(mp0,lp0)%Ti_k(1:ISPET,i1d)
        !B:
...
ELSE IF( r(1)==r(2) ) THEN
   print "('!R1=R2!',5E13.5,i6)",r(0:2),Qint(iR,i1d,imp,1:2),i
 IF( r(2)==(earth_radius+ht90) ) then
!dbg20111006: somehow IS does not fit here???
   print *,'i=IN/S',JMIN_IN(lp),JMAX_IS(lp),lambda_m(0:2)*180./pi
   x(0:2) = lambda_m(0:2)
 ELSE
...
!t        n0(jth,1:2)=Qint(jth,i1d,imp,1:2) * ksi_fac(1:2)**(4./3.)
!t      END IF
...
!t         ELSE IF ( jth==ISPEC+1 ) THEN 
!t           n0_1d%Te_k(            i1d) = ( (x(1)-x(0))*n0(jth,2) + (x(0)-x(2))*n0(jth,1) ) / ( x(1)-x(2) )
!t         ELSE 
!t           n0_1d%Ti_k(jth-ISPEC-1,i1d) = ( (x(1)-x(0))*n0(jth,2) + (x(0)-x(2))*n0(jth,1) ) / ( x(1)-x(2) )
!t         END IF
       ELSE
