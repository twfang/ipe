...
!TWFANG
      ...
      USE module_regrid2ipe, ONLY : regrid2ipe
      IMPLICIT NONE
!
...
!TWFANG
      INTEGER :: inIPE1d(NLP), isIPE1d(NLP)   ! 1d in, read from the IPE grid file
      real(KIND=real_prec),DIMENSION(kmlonp1,kmlat,2) :: ed_dyn
      real(KIND=real_prec),DIMENSION(2,NLP,NMP,2) :: ed_IPE !1:N/SH....4:ed1/2
      real(KIND=8), DIMENSION(NPTS2D) :: ipeGL,ipeGL_deg  
...
! add by TWFANG for feeding back the self-consistent electric fields
      if (sw_eldyn==0) then
...
!            write(*,*) 'ed1_90',ed1_90
!            write(*,*) 'ed2_90',ed2_90
         write(unit=2222,FMT='(20E12.4)') ed1dy
         write(unit=2222,FMT='(20E12.4)') ed2dy
!        write(unit=2223,FMT='(20E12.4)') ed1_90
!        write(unit=2223,FMT='(20E12.4)') ed2_90

      else if (sw_eldyn==1) then
...
!         ENDIF !( sw_exb_up<=1.and. ... ) 
      ...
      mlon_loop90km1: DO mp=1,NMP
        mlat_loop90km2: DO lp=1,NLP
...
!           (1) WACCM E empirical model
!           Ed1/2[V/m] at ( phi_t1(mp), theta_t1(lp) ), Be3[T]
!           note: Ed1_90, Ed2_90, and Be3 are constant along magnetic
!           field lines!!!
            midpoint = JMIN_IN(lp) + (JMAX_IS(lp) - JMIN_IN(lp))/2
...
! TWFANG TEST set limit to Ed1
!          if (lp>40.and.ED1_90(1,lp,mp)>0.0008) then
!           ED1_90(1,lp,mp)=0.0008
!           print *,'reset ED1',lp,mp,Ed1_90(1,lp,mp)
!          endif
...
!nm20130201: temporary solution to test the code
        ...
        END DO mlat_loop90km2 !: DO lp=1,NLP
      END DO mlon_loop90km1     !: DO mp=1,nmp
