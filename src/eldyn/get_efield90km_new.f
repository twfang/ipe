...
!TWFANG
      ...
      real(KIND=8), DIMENSION(NPTS2D) :: ipeGL  
...
! TWFANG
          else if (sw_eldyn==0) then
          write(*,*) 'works well!'
!         write(*,*) 'IN=',JMIN_IN_all
!         write(*,*) 'IS=',JMAX_IS_all
!            inIPE1d=JMIN_IN_all(mp,:)
!            isIPE1d=JMAX_IS_all(mp,:)
             inIPE1d=JMIN_IN
             isIPE1d=JMAX_IS
             write(*,*) 'inIPEid',inIPE1d,isIPE1d
             write(*,*) 'ed_dyn in get_efield90',ed1dy
             write(*,*) 'ed_dyn in get_efield90',ed2dy
             ed_dyn(:,:,1)=ed1dy
             ed_dyn(:,:,2)=ed2dy
             ipeGL=plasma_grid_GL(:,lp)
             write(*,*) 'ipeGL',ipeGL
!            call regrid2ipe(inIPE1d,isIPE1d,ipeGL,ed_dyn,ed_IPE)  
!            ed1_90=ed_IPE(:,:,:,1);
!            ed2_90=ed_IPE(:,:,:,2);
